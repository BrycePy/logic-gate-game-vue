...
class Terminal {
  ...
  remove() {
    if (this.isSource && this.parent) {
      this.parent.outputTerminals = this.parent.outputTerminals.filter(
        (t) => t !== this
      );
    } else {
      this.parent.inputTerminals = this.parent.inputTerminals.filter(
        (t) => t !== this
      );
    }
    this.relatedTerminals = [];
...
class Wire {
  ...
  remove() {
    this.terminalSrc.relatedTerminals =
      this.terminalSrc.relatedTerminals.filter((t) => t !== this.terminalSink);
    this.terminalSink.relatedTerminals =
      this.terminalSink.relatedTerminals.filter((t) => t !== this.terminalSrc);
...
class Gate {
  ...
  remove() {
    this.world.eventManager.publish("GATE_PRE_REMOVE", this);
    if (this.world.previousTerminal?.parent === this) {
...
class World {
  constructor(eventManager) {
    this.eventManager =
      eventManager === undefined ? new EventManager() : eventManager;
  ...
  get nonIOGates() {
    return this.gates.filter(
      (g) => !this.inputs.includes(g) && !this.outputs.includes(g)
    );
  }
  ...
  async evaluate(inputs, instant) {
    ...
    this.eventManager.publish("WORLD_RESULT", {
      inputs: inputs,
      results: results,
    });
    return results;
  ...
  enableAutoSleep() {
    // this should be called at the root world
    this.autoSleep = true;
  ...
  makeConnection(terminal) {
    ...
    if (!this.previousTerminal) {
      ...
      this.eventManager.publish("WORLD_TERMINAL_SELECTED", terminal);
    } else {
  ...
  clearSelection() {
    if(!this.previousTerminal) return;
    this.previousTerminal = null;
    this.eventManager.publish("WORLD_TERMINAL_SELECTED", null);
  }
  ...
  getWiresByTerminal(terminal) {
    return this.wires.filter(
      (w) => w.terminalSrc === terminal || w.terminalSink === terminal
    );
  }
  ...
  getWiresBetween(terminal1, terminal2) {
    return this.wires.filter(
      (w) =>
        (w.terminalSrc === terminal1 && w.terminalSink === terminal2) ||
        (w.terminalSrc === terminal2 && w.terminalSink === terminal1)
    );
  }
...
let functionSpecNAND = new LogicGateFunctionSpec(
  "NAND",
  (a, b) => !(a && b),
  2,
  1
...
let functionSpecNOR = new LogicGateFunctionSpec(
  "NOR",
  (a, b) => !(a || b),
  2,
  1
);
let functionSpecXOR = new LogicGateFunctionSpec(
  "XOR",
  (a, b) => (a || b) && !(a && b),
  2,
  1
);
let functionSpecXNOR = new LogicGateFunctionSpec(
  "XNOR",
  (a, b) => !((a || b) && !(a && b)),
  2,
  1
);
let functionSpecUnknown = new LogicGateFunctionSpec(
  "UNKNOWN",
  (a, b) => {
    false;
  },
...
let functionSpecUnknown1 = new LogicGateFunctionSpec(
  "UNKNOWN",
  (a) => {
    false;
  },
